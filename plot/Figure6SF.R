...
library(ggplot2)
library(dplyr)
l <- read.csv('~/Dropbox/fiction/final/allSF2016-01-02.csv')
...
marked <- c('uva.x000380956', '21631', '11711', '11305', 'mdp.39015004200997', 
            'dul1.ark+=13960=t41r7h934', 'uiuo.ark+=13960=t8x92v101', 'hvd.32044014279723', 'nyp.33433081958500')
...
p <- ggplot(l, aes(x = dateused, y = logistic, color = reviewed, shape = shape)) + 
  ...
  scale_y_continuous('Predicted probability of being science fiction\n', labels = percent, breaks = c(0.25,0.5,0.75)) + 
