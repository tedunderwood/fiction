...
p <- ggplot(data = variation, aes(x = as.factor(genre), y = accuracy)) + theme_bw() +
  ...
  scale_y_continuous('', labels = percent, breaks = seq(.76, .94, by = .02)) +
  theme(text = element_text(size = 20, family = "Baskerville"), panel.border = element_blank()) +
  theme(axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.title.x = element_blank())

tiff("/Users/tunder/Dropbox/active/criticalinquiry/boxplot2.tiff", height = 5, width = 6, units = 'in', res=400)
plot(p)
dev.off()
plot(p)
